...
### ⑤ 正規化（`06_warped.png` / `06b_remap_debug.png`）

外枠の曲がり具合によって 2 通りの処理を行います。
...
```
元画像上で 10×10 グリッド交点を検出
  → inv(persp_m) で正規化グリッドを元画像空間に逆投影し初期推定位置を計算
  → cornerMinEigenVal でコーナー応答マップを生成
  → パス 1: 太線交点（行・列 0,3,6,9）を広い窓で検出
  → パス 2: 双線形補間で残り交点の初期位置を推定し細線交点を検出
  → 検出した 10×10 交点を元に 81 セルを個別に getPerspectiveTransform + warpPerspective
  → 各セルを正規化正方形の対応位置に貼り合わせて square を構築
```

交点検出結果は `06b_remap_debug.png` に元画像上の赤丸でプロットされます。
...
正規化ステップの case に応じて 2 通りの処理を行います。

**case 1: 外枠がほぼ直線の場合**

```
正方形キャンバスに全解答数字を描画
  → warpPerspective の逆行列（inv_persp）で元画像座標系に一括変換
  → マスクを使って元画像に合成
```
...
```
各解答セルについて：
  → 交点座標（元画像空間）からセルの 4 頂点を取得
  → cell_img（正規化セルサイズの数字画像）を 4 頂点へ getPerspectiveTransform + warpPerspective
  → バウンディングボックスのみを対象にブレンドして元画像に直接描画
```

case 2 では逆 remap は不要で、検出した交点座標をそのまま描画先に使うため正確に重なります。
